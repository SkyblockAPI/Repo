...
const parseCraftingRecipe = (item, recipe) => {
    ...
    return {
        ...
        keys: keys
            .map(key => key.split(":"))
            .map(([id, amount]) => ({ id, count: parseInt(amount) })),
        pattern: pattern
            .map(key => {
                if (key === "") return -1;
...
export const Recipes = {
    ...
    write: () => {
        fs.writeFileSync("recipes.json", JSON.stringify(recipesFile, null, 4));
        fs.writeFileSync("cloudflare/recipes.min.json", JSON.stringify(recipesFile));
